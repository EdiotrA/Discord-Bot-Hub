...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(opt =>
      opt.setName('user').setDescription('The user to remove EXP from').setRequired(true)
    )
    .addIntegerOption(opt =>
      opt.setName('amount').setDescription('Amount of EXP to remove').setRequired(true).setMinValue(1).setMaxValue(1000000)
    ),
  ...
  async execute(interaction) {
    ...
    const removed = current.exp - newExp;
    const newLevel = ExpUtil.getLevel(newExp);

    await interaction.reply({
      embeds: [Embed.success(
        'EXP Removed',
        `Removed **${removed.toLocaleString()}** EXP from ${target}.`,
        [
